  ...
  if (isLoading) {
    return (
      <section className="rounded-lg border border-slate-800 bg-slate-900/60 p-4">
        ...
        <p className="mt-2 text-xs text-slate-500">Loading recent expenses…</p>
      </section>
    );
  }
  ...
  if (!expenses || expenses.length === 0) {
    return (
      <section className="rounded-lg border border-slate-800 bg-slate-900/60 p-4">
        ...
        <p className="mt-2 text-xs text-slate-500">No expenses recorded yet.</p>
      </section>
    );
  }
          ...
          return (
            <li key={expense.id} className="flex items-center justify-between gap-2 py-1.5">
              <div className="flex flex-col">
                <span className="text-[11px] font-medium text-slate-50">{description}</span>
